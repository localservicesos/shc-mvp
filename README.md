...
bun run lint            # eslint
bun run backup:db       # create a local Supabase/Postgres backup
bun run backup:list     # list local database backups
bun run backup:verify   # verify a backup file can be read
bun run backup:restore -- backups/database/<file>.dump --confirm-restore
bun run backup:schedule:install  # install daily 11:15 PM local backup
...
- [docs/database-docs/BACKUPS.md](docs/database-docs/BACKUPS.md) — manual Supabase Free plan backups
